...
Route::get('/count', 'CountController@index')->name('counter.index');
Route::delete('/count', 'CountController@reset')->name('counter.reset');

## Mailing

Route::middleware('auth')->get('/summary', function() {
    $user = Auth::user();
    $data = [
        'date' => Carbon::today()->format('d-m-Y'),
        'posts' => Post::today(),
    ];
    Mail::send('emails.summary', $data, function($message) use ($user, $data) {
        $message->to($user->email, $user->name)->subject('Post Summary ' . $data['date']);
    });
    return 'Summary sent to ' . $user->email;
});
